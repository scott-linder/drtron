/*=============================================================================
#     FileName: drtron.c
#         Desc: A small tron/worm clone in C using ncurses
#       Author: Scott Linder
#        Email: scott <at> hamstercafe.com
#     HomePage: www.hamstercafe.com
#      Version: 0.0.1
#   LastChange: 2013-01-26 17:30:15
#      History:
=============================================================================*/
...
{
    //Start by getting declaring variables not linked to any one game
    settings_t settings;   //Same settings struct passed around for life of program
    enum playgame_ret game_term;//Game return to indicate action to take
                                // REPEAT = new game with same settings
                                // NEW = new game with new settings
    ...
    attron(COLOR_PAIR(1));
    mvprintw(LINES / 2, (COLS - 6) / 2, "DrTron");
    ...
    while (game_term != EXIT)
    {
        ...
        {
            //Let user input new settings
            get_new_settings(&settings);
            //Start the new game
            game_term = play_game(&settings);
        }
        else//Just exit if we don't understand the response
        {
            break;
        }
    }
...
void cleanup(settings_t *settings)
{
    int i;
    for (i=0; i < MAX_PLS; i++)
    {
